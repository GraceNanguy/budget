...
export default function IncomePage() {
    ...
    const unsubscribeAuth = auth.onAuthStateChanged(async (user) => {
      if (user) {
        ...
        try {
          ...
          const q = query(transactionsRef, where("userId", "==", user.uid), where("type", "==", "income"));
  ...
  const handleAddTransaction = async (data: Partial<Transaction>) => {
    ...
    try {
      console.log("Données reçues dans handleAddTransaction (income):", data);
        ...
        <div className="grid gap-4 md:gap-8 max-w-6xl mx-auto">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-muted-foreground">Gérez et suivez tous vos revenus</p>
            </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-sm text-muted-foreground">
              Total: <span className="font-semibold text-green-500">{formatAmount(totalIncome)}</span>
            </div>
          </div>
          ...
          {isLoading ? (
            ...
            <Card>
              <CardHeader>
                <CardTitle>Tous les revenus</CardTitle>
                <CardDescription>Liste complète de vos revenus</CardDescription>
              </CardHeader>
              <CardContent>
                <TransactionList
